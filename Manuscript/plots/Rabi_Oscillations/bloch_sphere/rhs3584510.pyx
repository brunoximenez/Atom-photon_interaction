...
cdef complex spectral0(double w, double t): return 0.05 * (w >= 0)
cdef complex spectral2(double w, double t): return 0.05 * (w >= 0)
cdef complex spectral3(double w, double t): return 0.05 * (w >= 0)
...
def cy_td_ode_rhs(
    ...
    cdef complex[::1, :] Ac3 = farray_alloc(nrows)
    dense_add_mult(Ac3, A3, 1)
    dense_add_mult(Ac3, A4, 1)
    br_term_mult(t, Ac3, evecs, skew, dw_min, spectral3, eig_vec, out, nrows, 1, 0.1, 1e-12)
    PyDataMem_FREE(&Ac3[0,0])
